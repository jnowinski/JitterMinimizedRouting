...
if [ "$1" == "--debug_all" ]; then
  ...
  ./waf configure --disable-werror --build-profile=optimized --enable-mpi --out=build/optimized || exit 1

elif [ "$1" == "--optimized_with_tests" ]; then
  ./waf configure --disable-werror --build-profile=optimized --enable-mpi --enable-tests --out=build/optimized_with_tests || exit 1
