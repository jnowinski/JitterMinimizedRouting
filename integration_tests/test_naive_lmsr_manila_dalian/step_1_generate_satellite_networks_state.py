...
# Forwarding state with naive LMSR algorithm
print("Generating forwarding state with naive LMSR routing...")
...
print("\n" + "="*60)
print("BENCHMARK RESULTS - Naive LMSR")
print("="*60)
print(f"Total execution time: {elapsed:.2f} seconds ({elapsed/60:.2f} minutes)")
print(f"Time per timestep: {elapsed/100:.3f} seconds")
print(f"Algorithm: algorithm_lmsr (all satellites)")
print(f"Complexity: O(N × V² × (V log V + E))")
print("="*60)

# Write timing to file
with open("temp/gen_data/benchmark_naive_lmsr.txt", "w") as f:
    f.write(f"Algorithm: Naive LMSR\n")
