...
# Use free_gs_one_sat_many_only_over_isls algorithm
...
# Forwarding state with free_gs_one_sat_many_only_over_isls algorithm
print("Generating forwarding state with standard routing...")
...
print("\n" + "=" * 60)
print("BENCHMARK RESULTS - free gs one sat many only over ISLs")
print("=" * 60)
print(f"Total execution time: {elapsed:.2f} seconds ({elapsed / 60:.2f} minutes)")
print(f"Time per timestep: {elapsed / 100:.3f} seconds")
print(f"Algorithm: algorithm_free_gs_one_sat_many_only_over_isls (60 anchors)")
print(f"Complexity: O(N × (V log V + E))")
print("=" * 60)

# Write timing to file
with open("temp/gen_data/benchmark_standard_routing.txt", "w") as f:
    f.write(f"Algorithm: free gs one sat many only over ISLs\n")
